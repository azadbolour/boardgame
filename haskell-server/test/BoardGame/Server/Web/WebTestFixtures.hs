...
makeGame :: GameEnv -> GameParams -> [PiecePoint] -> [Piece] -> [Piece] -> [[Int]] -> IO StartGameResponse
makeGame env gameParams initialPiecePoints userTrayStartsWith machineTrayStartsWith pointValues =
  satisfiesRight
    =<< runExceptT (startGameHandler env (StartGameRequest gameParams initialPiecePoints userTrayStartsWith machineTrayStartsWith pointValues))
